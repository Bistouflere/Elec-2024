...
<div class="encadre">
    ...
    <p>L'ampoule a consommé au total 0kW aujourd'hui</p>
...
function updateLightTime() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/light_time", true);
    xhr.onload = function() {
        if (xhr.status == 200) {
            document.getElementById("light_time").innerText = xhr.responseText + "s";
        }
    };
    xhr.send();
}
...
def handle_request(client_socket):
    global current_color, current_timer, current_watts_value, current_max_value
    try:
